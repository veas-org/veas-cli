...
export async function createPAT(): Promise<void> {
  try {
    ...
    console.log(pc.cyan(`claude mcp add veas -e VEAS_PAT="${result.token}" -- node "${process.cwd()}/bin/veas.js" serve`));
  } catch (error) {
...
export async function listPATs(): Promise<void> {
  try {
    ...
    const { tokens } = await response.json() as { tokens: Array<{ id: string; name: string; created_at: string; last_used_at?: string }> };
