...
export async function configureForClaude(): Promise<void> {
  try {
    ...
    console.log(
      pc.green(`claude mcp add veas -e VEAS_API_URL=https://your-api.com -- node "${cliPath}" serve --port 3333`),
    )
    console.log('')
...
export async function testMCPConnection(): Promise<void> {
  try {
    ...
    let result:
      | { error?: { message: string }; result?: { tools?: Array<{ name: string; description: string }> } }
      | undefined
...
export async function testConnection(): Promise<void> {
  ...
  try {
    ...
    const token =
      (credentials as { accessToken?: string; token?: string })?.accessToken ||
      credentials?.token ||
