...
describe('MCPServer', () => {
  ...
  describe('initialize', () => {
    ...
    it('should throw error when no token available', async () => {
      vi.mocked(authWrapper.getBestAuthToken).mockRejectedValueOnce(new Error('No authentication token found'))

      await expect(server.initialize()).rejects.toThrow('No authentication token found')
    })
  })
  ...
  describe('request handlers', () => {
    ...
    describe('CallToolRequest', () => {
      ...
      it.skip('should execute tool and cache result on cache miss', async () => {
        ...
        vi.mocked(global.fetch).mockResolvedValueOnce(mockFetchResponse({ result: responseData }))
        ...
        expect(mockCacheManager.set).toHaveBeenCalledWith('project_list', { limit: 10 }, responseData)
  ...
  describe('executeTool', () => {
    ...
    it.skip('should use environment variable for API URL', async () => {
      ...
      vi.mocked(global.fetch).mockResolvedValueOnce(mockFetchResponse({ result: { success: true } }))

      const result = await (server as any).executeTool('test_tool', { arg: 'value' })

      expect(global.fetch).toHaveBeenCalledWith('https://api.veas.com/api/mcp-manual', expect.any(Object))

      expect(result).toEqual({ success: true })
    })

    it('should handle JSON-RPC error response', async () => {
      vi.mocked(global.fetch).mockResolvedValueOnce(mockFetchResponse({ error: { message: 'Invalid parameters' } }))

      await expect((server as any).executeTool('test_tool', {})).rejects.toThrow('Invalid parameters')
    })
  })
})
