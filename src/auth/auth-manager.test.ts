...
describe('AuthManager', () => {
  ...
  describe('login', () => {
    it('should successfully login and store encrypted credentials', async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith(expectedAuthFile, expect.any(String), 'utf-8')
    ...
    it('should throw error on authentication failure', async () => {
      ...
      await expect(authManager.login(email, password)).rejects.toThrow('Authentication failed: Invalid credentials')
    })
    ...
    it('should throw error when no user or session returned', async () => {
      ...
      await expect(authManager.login(email, password)).rejects.toThrow('Invalid response from authentication server')
    })
  })
  ...
  describe('encryption/decryption', () => {
    ...
    it('should throw error for invalid encrypted format', () => {
      ...
      expect(() => authManagerAny.decrypt('invalid-format')).toThrow('Invalid encrypted data format')
    })
  })
})
