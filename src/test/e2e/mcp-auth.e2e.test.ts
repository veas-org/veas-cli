...
describe.skip('MCP Authentication E2E Tests', () => {
  ...
  describe('Personal Access Token (PAT) Authentication', () => {
    ...
    it('should fail with invalid PAT format', async () => {
      ...
      await expectMCPError(client.listTools(), 'Invalid token format')
    })
  })
  ...
  describe('CLI Token Authentication', () => {
    ...
    it('should fail with expired CLI token', async () => {
      ...
      await expectMCPError(client.listTools(), 'expired')
    })
  })
  ...
  describe('Tool Execution with Authentication', () => {
    ...
    it('should fail tool execution without authentication', async () => {
      ...
      // No token set
      await expectMCPError(client.callTool('mcp_get_user_info', {}), 'authentication')
    })
  ...
  describe('Token Validation', () => {
    it('should handle missing token gracefully', async () => {
      ...
      // Don't set any token
      await expectMCPError(client.listTools(), 'Missing authentication token')
    })
    ...
    it('should handle malformed tokens', async () => {
      ...
      const malformedTokens = ['', ' ', 'Bearer token', 'null', 'undefined']

      for (const token of malformedTokens) {
        client.setToken(token)
        await expectMCPError(client.listTools(), /(Invalid|Missing|authentication)/i)
      }
    })
  })
})
